...
class AuthenticatedVoter implements VoterInterface {
    ...
    public function vote(TokenInterface $token, $object, array $attributes) {
        ...
        foreach ($attributes as $attribute) {
            ...
            if (self::IS_AUTHENTICATED_ANONYMOUSLY === $attribute && ($this->authenticationTrustResolver->isAnonymous($token) || $this->authenticationTrustResolver->isRememberMe($token) || $this->authenticationTrustResolver->isFullFledged($token))) {
                return VoterInterface::ACCESS_GRANTED;
            }
        }
