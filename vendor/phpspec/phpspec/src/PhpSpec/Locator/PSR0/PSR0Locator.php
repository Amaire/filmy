...
class PSR0Locator implements ResourceLocatorInterface {
     ...
     */
    public function __construct($srcNamespace = '', $specNamespacePrefix = 'spec', $srcPath = 'src', $specPath = '.', Filesystem $filesystem = null, $psr4Prefix = null) {
        ...
        if ($sepr === $this->srcPath) {
            throw new InvalidArgumentException(sprintf(
                    'Source code path should be existing filesystem path, but "%s" given.', $srcPath
            ));
        }

        if ($sepr === $this->specPath) {
            throw new InvalidArgumentException(sprintf(
                    'Specs code path should be existing filesystem path, but "%s" given.', $specPath
            ));
        }
    }
    ...
    public function supportsQuery($query) {
        ...
        return 0 === strpos($path, $this->srcPath) || 0 === strpos($path, $this->specPath)
        ;
    }
    ...
    public function supportsClass($classname) {
        ...
        return '' === $this->srcNamespace || 0 === strpos($classname, $this->srcNamespace) || 0 === strpos($classname, $this->specNamespace)
        ;
    }
    ...
    private function createResourceFromSpecFile($path) {
        ...
        if (0 !== strpos($classname, $specNamespace)) {
            throw new \RuntimeException(sprintf(
                    'Spec class must be in the base spec namespace `%s`.', $this->getSpecNamespace()
            ));
        }
