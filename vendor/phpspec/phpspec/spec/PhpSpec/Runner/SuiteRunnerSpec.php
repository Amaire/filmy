...
class SuiteRunnerSpec extends ObjectBehavior {
    ...
    function it_dispatches_events_before_and_after_the_suite($suite, $dispatcher) {
        ...
        $dispatcher->dispatch('beforeSuite', Argument::type('PhpSpec\Event\SuiteEvent')
        )->shouldHaveBeenCalled();

        $dispatcher->dispatch('afterSuite', Argument::type('PhpSpec\Event\SuiteEvent')
        )->shouldHaveBeenCalled();
    }
    ...
    function it_dispatches_afterSuite_event_with_result_and_time($suite, $specRunner, $dispatcher) {
        ...
        $dispatcher->dispatch('afterSuite', Argument::that(
                        function ($event) {
                    return ($event->getTime() > 0) && ($event->getResult() == ExampleEvent::FAILED);
                }
                )
        )->shouldHaveBeenCalled();
    }
